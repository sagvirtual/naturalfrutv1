...
$iditemr = $_POST['iditemr'];

//if(empty($editar)){
/* controlo si no esta */
$sqlprecrod = mysqli_query($rjdhfbpqj, "SELECT * FROM prodcom Where id_orden='$id_orden' and id_producto = '$id_producto' and modo = '1'");
if ($rowprecioprod = mysqli_fetch_array($sqlprecrod)) {
    if ($agrstock > 0) {
        $sqlpredoprod = "Update prodcom Set fecven='$fecven', agrstock='$agrstock', costoxcaj='$costoxcaj', costo='$costo', tipo='$tipo', descuento='$descuento', pcondescu='$pcondescu', iibb_bsas='$iibb_bsas', iibb_caba='$iibb_caba', perc_iva='$perc_iva', iva='$iva', pbruto='$pbruto', desadi='$desadi', costo_total='$costo_total', costoxcaja='$costoxcaja', cpratotal_prod='$cpratotal_prod', fecha='$fecha' Where id = '$iditemr' and  cerrado='0' and id_producto!='0'";
        mysqli_query($rjdhfbpqj, $sqlpredoprod) or die(mysqli_error($rjdhfbpqj));
    }
} else {/* inserto */
    if ($agrstock > 0) {
