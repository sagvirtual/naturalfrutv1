...
if(!empty($_POST['id_cliente']) && $_POST['id_cliente']!=' ' && $_POST['id_cliente']!=null)
  ...
  $sqlordenc = mysqli_query($rjdhfbpqj, "SELECT * FROM proveedores Where  $condicion_fincl  and estado='0' and rubro='0' ORDER BY `proveedores`.`empresa` ASC Limit 5");
  
   while ($rowordencl = mysqli_fetch_array($sqlordenc)) {
           
            
            $empresa=$rowordencl["empresa"];


 echo '
 <p class="cuadbus">' . $empresa . '<i style="display:none;">@'.$rowordencl["id"].'@</i></p>
