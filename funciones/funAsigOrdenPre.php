...
function AsigOrdenHojaPre($rjdhfbpqj,$id_usuarioclav) {
    ...
    if ( $rowhoja = mysqli_fetch_array( $sqlhoja ) ) {
         ...
         $sqlclorden = mysqli_query( $rjdhfbpqj, "SELECT * FROM orden WHERE id_hoja='$id_hoja' and  id_usuarioclav='$id_usuarioclav' and  picking='0' and col='2' ORDER BY `orden`.`position` DESC");
         if ( $rowclorden = mysqli_fetch_array( $sqlclorden ) ) {

         $idordenf = $rowclorden[ 'id' ];
