...
if (count($palabrascl) === 0 || (count($palabrascl) === 1 && $palabrascl[0] === '')) {
    $condicioncl[] = "CONCAT(c.nom_empr, ' ', c.nom_contac, ' ', c.address) LIKE '%%'";
} else {
...
if ($condicion_fincl === '') {
    $condicion_fincl = "CONCAT(c.nom_empr, ' ', c.nom_contac, ' ', c.address) LIKE '%%'";
}
...
$sqlclientes = mysqli_query($rjdhfbpqj, "
    SELECT 
        c.nom_empr, c.address, c.id, c.nom_contac, c.wsp, c.localidad,
        lc.ultima_compra,
        CASE 
