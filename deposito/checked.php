...
if ($rowrdep = mysqli_fetch_array($rowrdep)) {
    ...
    if ($fecha_ing == $fecha_venc || "3000-00-00" == $fecha_venc) {
        $fecha_venc = "0000-00-00";
    }
...
$sqwden = mysqli_query($rjdhfbpqj, "SELECT * FROM deposito Where id_producto='$unidsx' and fin='0' ORDER BY `fecha_venc` ASC");
if ($rodwod = mysqli_fetch_array($sqwden)) {
    $id_palletdfeh = $rodwod["id_pallet"];
    $idcont += 1;


    if ($id_palletdfeh == $id_palletb) {
        $colorbla = 'success';
        $block = '<img src="../deposito/estado0.png" style="width: 100px;"><p style="color:green">Habilitado</p>';
    } else {
        $colorbla = 'danger';
        $block = '<img src="../deposito/estado0.png" style="width: 100px;"><p style="color:green">Habilitado</p>';
        // $block='<img src="../deposito/estado1.png" style="width: 100px;"><p style="color:red">No tocar</p>';
    }
}
        ...
        <div class="row">
            ...
            <div class="col-lg-12" style="padding-top: 10px;">
                ...
                <div class="alert alert-success text-center">
                    <strong>Pica el Codigo del Pallet para verificar</strong>
                </div>
                ...
                <br>
                <? if ($id_usuarioclav == '40'  || $id_usuarioclav == '93') { ?>
                    <div class="text-center">
                        <button type="button" class="btn btn-danger" onclick="ajax_pallets('<?= $id_pallet ?>');">Omitir verificar
                        </button>
                    </div>
                <? } ?>
...
<script>
    function ajax_pallets(id_pallet) {
        ...
        var id_palletdo = id_pallet;
        if (id_pallet.length === 12) {
            var id_palletb = <?= $id_palletb ?>;
            if (id_palletdo == id_palletb) {
                location.href = '../deposito/ingreso?id_pallet=' + id_palletdo + '&idproedit=<?= $unidsx ?>&odoin=2&modbaja=<?= $modbaja ?>';
            } else {

                alert("Pallet Incorrecto");
                location.href = '../deposito/checked.php?id_pallet=<?= $id_palletb ?>&modbaja=<?= $modbaja ?>';
            }
        }
