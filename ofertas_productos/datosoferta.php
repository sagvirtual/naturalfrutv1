...
$sql = mysqli_query($rjdhfbpqj, "SELECT * FROM oferta Where id_producto = '$id_producto'");
if ($row = mysqli_fetch_array($sql)) {
    $sqlpreeprod = "Update oferta Set descuento='$descuento',cantmax='$canmax',unidad='$unidad',activo='$activo',fecha_desde='$fecha_desde',fecha_hasta='$fecha_hasta',activo='$activo',stock='$stock',unidstock='$unidstock',nota='$nota',stockhistorial='$stockhistorial',oferta='$oferta',limite='$limite',dosporeuno='$dosporeuno',forzado='$forzadoprecio' Where id_producto = '$id_producto'";
    mysqli_query($rjdhfbpqj, $sqlpreeprod) or die(mysqli_error($rjdhfbpqj));
} else {

    $sqlins = "INSERT INTO `oferta` (descuento, cantmax, unidad, activo, fecha_desde, fecha_hasta, id_producto,stock,unidstock,nota,stockhistorial,oferta,limite,dosporeuno,forzado) VALUES ('$descuento', '$canmax', '$unidad', '$activo', '$fecha_desde', '$fecha_hasta', '$id_producto', '$stock', '$unidstock', '$nota', '$stockhistorial', '$oferta', '$limite', '$dosporeuno', '$forzadoprecio');";
    mysqli_query($rjdhfbpqj, $sqlins) or die(mysqli_error($rjdhfbpqj));
}
$origen = "Oferta," . $fecha_desde . "," . $fecha_hasta . "," . $descuento . "," . $canmax . "," . $unidad . "," . $stock . "," . $unidstock . "," . $dosporeuno;
