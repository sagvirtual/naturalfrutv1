...
<div id="cudrobus" class="fonbus">
    ...
    if (!empty($_POST['producto']) && $_POST['producto'] != ' ' && $_POST['producto'] != null)
        ...
        $sqlorden = mysqli_query($rjdhfbpqj, "SELECT id, nombre, unidadnom FROM productos Where  $condicion_final ORDER BY `productos`.`nombre` ASC Limit 10");
    while ($roworden = mysqli_fetch_array($sqlorden)) {
        ...
        $nombrevein = "<b>" . $nombrevd[0] . "</b>  <br>" . $empresamarca . " [Stock: " . $stockdispom . " " . $roworden['unidadnom'] . "]";

        echo '
        <p class="cuadbus" style="padding-left:10px;">' . $nombrevein . '<i style="display:none;">[' . $roworden["id"] . '@' . $roworden["unidadnom"] . '@' . $id_ordenpro . '@</i></p>
