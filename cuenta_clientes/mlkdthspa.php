...
if(!empty($idcod)){
    ...
    FROM item_orden WHERE id='$idse' and modo='1'";
    
    if ($rjdhfbpqj->query($sql_historial) === TRUE) {
        // Si se guardó en el historial, proceder con el borrado
       // $sqlborr = "DELETE FROM item_orden WHERE id='$idse' and modo='1'";
...
echo ("top.location.href='/cuenta_clientes/?fort=1&hfbbd=$idcliente'");
echo ("</script>");
?>
