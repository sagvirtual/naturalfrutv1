...
$pagoblannegro = $_GET['tipocuneta']; //2blanco     
$fechapago = $_GET['fechapago']; //2blanco
...
function listopagos($rjdhfbpqj, $id_orden, $id_cliente, $pagoblannegro, $fechapago)
{
    $sqlpag = mysqli_query($rjdhfbpqj, "SELECT * 
        FROM prodcom 
        WHERE modopag='1' AND tipocuneta='$pagoblannegro' AND valor > 0 AND tipopag != '33' AND id_orden='$id_orden' AND id_proveedor='$id_cliente' and fecha='$fechapago' 
        ORDER BY id DESC");
...
if (!empty($id_cliente)) {
    ...
    $sqlpag = mysqli_query($rjdhfbpqj, "SELECT valor, fecha,nref
        FROM prodcom 
        WHERE modopag='1' AND tipocuneta='$pagoblannegro' AND valor > 0  AND tipopag != '33' AND id_orden='$id_orden' AND id_proveedor='$id_cliente' and fecha='$fechapago' ");
    ...
    $sqlpasgfa = mysqli_query($rjdhfbpqj, "SELECT valor, fecha,nref
        FROM prodcom 
        WHERE modo='$pagoblannegro' AND id_orden='$id_orden' AND nref!=''");

    if ($rowpag = mysqli_fetch_array($sqlpasgfa)) {
        $nref  = 'Fac Nº ' . $rowpag['nref']; // última fecha encontrada
    }
    ...
    $sqlclie = mysqli_query($rjdhfbpqj, "SELECT empresa, address FROM proveedores WHERE id='$id_cliente'");
    if ($rowcli = mysqli_fetch_array($sqlclie)) {
        $nom_empr   = $rowcli['empresa'];
        $address    = $rowcli['address'];
    }

    // Traigo / creo el número de recibo
    $formato = '';
    $sql = mysqli_query($rjdhfbpqj, "SELECT id, fecha FROM recibo WHERE id_orden='$id_orden' AND id_cliente='$id_cliente' AND fecha='$fechapago'  and origen='0' LIMIT 1");
    if ($row = mysqli_fetch_array($sql)) {
        ...
        $sqlIns = "INSERT INTO recibo (id_orden, id_cliente, fecha) VALUES ('$id_orden', '$id_cliente', '$fechapago')";
    ...
    function render_copia($tipoCopia, $formato, $fecharec, $nom_empr, $nom_contac, $address, $localidad, $numfactu, $todag, $totalpag, $rjdhfbpqj, $id_orden, $id_cliente, $pagoblannegro, $fechapago, $nref)
    {
?>
        <section class="recibo">
            <header class="marca">
                ...
                <div class="datos-empresa">
                    ...
                    <div class="sub">
                        Rocha 1935, B1650 Villa Lynch,<br>Provincia de Buenos Aires
                    </div>
                </div>
                <div class="info-doc">
            ...
            <div class="grid">
                <div class="card">
                    <h4>Pago realizado a:</h4>
                    <div class="fila">
                        <div class="et">Nombre / Razón Social:</div>
                        <div class="val"><?= htmlspecialchars($nom_empr) ?></div>
                    </div>
                    <div class="fila">
                ...
                <div class="card">
                    <h4>Medio de pago</h4>
                    <?php listopagos($rjdhfbpqj, $id_orden, $id_cliente, $pagoblannegro, $fechapago); ?>
                </div>
            </div>
    ...
    <body>
        <div class="wrap">
            ...
            <?php
            // ========= ORIGINAL =========
            render_copia('ORIGINAL',  $formato, $fecharec, $nom_empr, $nom_contac, $address, $localidad, $numfactu, $todag, $totalpag, $rjdhfbpqj, $id_orden, $id_cliente, $pagoblannegro, $fechapago, $nref);
            ?>
            ...
            <?php
            // ========= DUPLICADO =========
            render_copia('DUPLICADO', $formato, $fecharec, $nom_empr, $nom_contac, $address, $localidad, $numfactu, $todag, $totalpag, $rjdhfbpqj, $id_orden, $id_cliente, $pagoblannegro, $fechapago, $nref);
            ?>
        </div>
    </body>
