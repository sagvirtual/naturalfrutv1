...
foreach ($palabrascl as $palabracl) {
    ...
    $condicioncl[] = "empresa LIKE '$terminocl'";
    //$condicioncl[] = "empresa LIKE '$terminocl' Or address LIKE '$terminocl'";
}
...
if(!empty($_POST['id_cliente']) && $_POST['id_cliente']!=' ' && $_POST['id_cliente']!=null)
  ...
  $sqlordenc = mysqli_query($rjdhfbpqj, "SELECT empresa,address,id FROM proveedores Where  $condicion_fincl  and estado='0' ORDER BY `proveedores`.`empresa` ASC Limit 5");
  
   while ($rowordencl = mysqli_fetch_array($sqlordenc)) {
           
            
            $nombrevc=$rowordencl["empresa"];
 ...
 echo '
 <p class="cuadbus">'.$nombrevc.'<i style="display:none;">@'.$rowordencl["id"].'@</i></p>
