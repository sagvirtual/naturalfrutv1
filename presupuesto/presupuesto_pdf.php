...
<?php
$sqlordenx = mysqli_query($rjdhfbpqj, "SELECT * FROM presupuesto Where id='$idorden'");
...
$sqlwden = mysqli_query($rjdhfbpqj, "SELECT * FROM item_presupues Where id_orden = '$idorden' and modo='0'");
...
$sqlpeufpd = mysqli_query($rjdhfbpqj, "SELECT * FROM item_presupues Where id_cliente = '$id_clienteint'  and modo='1' and id_orden='$idorden'");
    ...
    <tr>
        <td
            ...
            <p style="font-size: 14px; font-weight: bold;">
                ...
                Nº <?= str_pad($idorden, 8, '0', STR_PAD_LEFT) ?><br><br>
...
<table style="vertical-align: top; width:100%; font-size: 12px; border-collapse: collapse;">
    <tr style="vertical-align: middle;line-height: 1.5;height: 10mm;font-weight: bold;">
        ...
        <td style="width:70px;text-align:right;padding-left:5px;"><strong>Precio&nbsp;Unit.</strong></td>
...
while ($roworden = mysqli_fetch_array($sqlorden)) {
    ...
    $preciounit = $roworden['total'] / $roworden['kilos'];
...
<td style="width:70px;text-align:right;padding-right: 5px; padding-top: 5px; padding-bottom: 5px;">$' . number_format($preciounit, 2, ',', '.') . '</td>
<td style="width:70px; text-align:right;padding-top: 5px; padding-bottom: 5px;padding-right: 5px;">$' . number_format($roworden['total'], 2, ',', '.') . '</td>
</tr>
</table>
                            ...
                            <tr>
                                <td>
                                    <p style="font-size: 10px;  line-height: 1.5;">
                                        Presupuesto valido hasta <?= date('d/m/Y', strtotime($fechahoy . ' +7 days')) ?>, sujeto a posibles modificaciones sin previo aviso.
                                    </p>
                                </td>
                            </tr>
                        </table>
...
$mpdf->SetHTMLFooter('<hr><table style="width:280mm;"><tr><td style="width:50%;"><div class="page-footer"><i>Página: {PAGENO} de {nbpg} - Presupuesto: Nº ' . $idorden . ' - Cliente: ' . $zonanom . ' - ' . $nom_empr . '</i></div></td></tr></table>');
...
$mpdf->writeHTML($html);
$mpdf->Output('Presupuesto No ' . $idorden . ' Fecha ' . $fechaord . '.pdf', 'I'); //D
