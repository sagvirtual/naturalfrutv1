...
if(!empty($_POST['producto']) && $_POST['producto']!=' ' && $_POST['producto']!=null)
  ...
  $sqlorden = mysqli_query($rjdhfbpqj, "SELECT * FROM productos Where  $condicion_final ORDER BY `productos`.`position` ASC Limit 35");
   while ($roworden = mysqli_fetch_array($sqlorden)) {
            ...
            $marcno=$nombrevd[1];
            $stockdispom= SumaStock($rjdhfbpqj,$id_productort);
            $marcno=$nombrevd[1];
            ...
            $nombrevein="<b>".$nombrevd[0]."</b>  ".$empresamarca." - ".$modo_producto."&nbsp;x&nbsp;".$kilo."&nbsp;".$unidadnom."<br>[Stock: ".$stockdispom." ".$roworden['unidadnom']." / ".$stokbulto[0]." ".$modo_producto."]";

 echo '
 <p class="cuadbus">' . $nombrevein . ' <i style="display:none;">@'.$roworden["unidadnom"].'@'.$roworden["id"].'@</i></p>
