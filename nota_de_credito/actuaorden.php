...
include('../lusuarios/login.php');
 ...
 $ordedecompra=$_POST['ordedecompra']; /* total ventas */ 

 if(!empty($idorden)){
 $sqlclientes = "Update nota_credito Set  subtotal='$subtotal', uniddesc='$uniddesc', desporsen='$desporsen', descuento='$descuento', perporsen='$perporsen', totalper='$totalper', ivaporsen='$ivaporsen', totalivas='$totalivas', total='$total', anterior='$anterior', pago='$pago', saldo='$saldo', adicional='$adicional', adicionalvalor='$adicionalvalor', observacion='$observacion' Where id = '$idorden'";
 mysqli_query($rjdhfbpqj, $sqlclientes) or die(mysqli_error($rjdhfbpqj));

 $sqlclientes = "Update item_orden Set  valor='$saldo'  Where id_notacredito = '$idorden'";
 mysqli_query($rjdhfbpqj, $sqlclientes) or die(mysqli_error($rjdhfbpqj));
