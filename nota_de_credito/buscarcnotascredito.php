                        ...
                        <table id="default-datatable" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 80px;" class="text-center">Nº&nbsp;Nota&nbsp;de&nbsp;Credito</th>
                                    <th>Cliente</th>
                                    <th style="width: 100px;">Ref.&nbsp;Nº&nbsp;Orden</th>
                            ...
                            <tbody>
                                ...
                                $sqluorden = mysqli_query($rjdhfbpqj, "SELECT e.id, e.fecha,e.ordedecompra, e.col, e.saldo, e.id_cliente, u.nom_empr, u.nom_contac, u.retira, u.zona, u.address, u.localidad, u.id as nuncliente
                                    FROM nota_credito e 
                                    INNER JOIN clientes u 
                                    ON e.id_cliente = u.id
                                    Where $qlovus e.col >= '0'  ORDER BY `fecha`  ASC $limit
                                    ");
                                ...
                                while ($rowuorden = mysqli_fetch_array($sqluorden)) {
                                    ...
                                    $ordedecompra = $rowuorden["ordedecompra"];
                                    if ($ordedecompra > 0) {
                                        $ordedecompra = $rowuorden["ordedecompra"];
                                    } else {
                                        $ordedecompra = "";
                                    ...
                                    if ($colestado == '0') {
                                        $estadocol = '<span class="badge" style="background-color:#AD3B06; color:white;">Ingresando</span>';
                                    } else {
                                        $estadocol = '<span class="badge">Realizado</span>';
                                    }
                                    ...
                                    $sqlpagdeufp = mysqli_query($rjdhfbpqj, "SELECT * FROM nota_credito Where id_cliente = '$id_cliente'and col>='0' ORDER BY `nota_credito`.`id` DESC");
                                    ...
                                    echo '
                                          <tr>
                                          <td style="color: black;">' . $estadocol . '</td>
                                    ...
                                    if ($Idordenultima == $Idordenultima) {
                                        echo '<a href="../nota_de_credito/?jhduskdsa=' . $id_clientecod . '&orjndty=' . $id_ordencod . '&ref=1">';
                                    } else {
                                        echo '<a href="../nota_de_credito/nota_de_credito_pdf?jdhsknc=' . $id_ordencod . '"  target="_blank" tabindex="-1" title="PDF Nota de Pedido">';
                                    }

                                    echo ' <button type="button" class="btn btn-secondary" style="background-color: #EAE9E9;color: black; font-weight: bold;">Nº ' . str_pad($id_orden, 8, '0', STR_PAD_LEFT) . '</button></td>   
                                             </a>
                                              <td>
                                              ' . $nombrezona . ' - ' . $nomclientes . ' ' . $nomnegocio . ' ' . $localidad . '
                                             
                                             <td class="text-center">Nº ' . $ordedecompra . '</td> 
                                             <td style="text-align: right;" class="text-center">$' . number_format($saldo, 0, ',', '.') . '</td>
                                           
                                              <td class="text-center"><a href="../nota_de_credito/nota_de_credito_pdf.php?jdhsknc=' . $id_ordencod . '" class="btn btn-dark-rgba" target="_blank" tabindex="-1" title="PDF Nota de Credito"><i class="dripicons-print"></i></a></td>
                                    ...
                                    if ($Idordenultima == $Idordenultima) {
                                        echo '
                                                    ...
                                                    <a href="../nota_de_credito/?jhduskdsa=' . $id_clientecod . '&orjndty=' . $id_ordencod . '&ref=1" class="btn btn-success-rgba" title="Editar"><i class="ri-pencil-line"></i></a>';
                                    }
                                    if ($colestado != '8') {
                                        echo ' 
                                                        <a href="javascript:void(0);" ondblclick="eliminar(' . "'/nota_de_credito/mlkdths?" . 'jnnfsc=' . $id_ordencod . '' . "'" . ')" class="btn btn-danger-rgba">
