...
if ($id_cliente > 1 && !empty($idproduc) && $importtot != 0) {
    ...
    if ($id_ordenedit == 'dsds' || $id_ordenedit == '') {
        $sqlordenx = mysqli_query($rjdhfbpqj, "SELECT * FROM nota_credito Where id_cliente = '$id_cliente' and finalizada='0' and fin='1'");
        if ($rowordenx = mysqli_fetch_array($sqlordenx)) {
            ...
            $sqlorden = "INSERT INTO `nota_credito` (id_cliente, fecha, hora, anclaje, fin, zona, poszona, id_usuarioclav,ordedecompra) VALUES ('$id_cliente', '$fechaorden', '$horaord', '$anclaje', '1', '$zona', '$poszona', '$id_usuarioclav', '$ordedecompra');";
            mysqli_query($rjdhfbpqj, $sqlorden) or die(mysqli_error($rjdhfbpqj));


            $sqlordenty = mysqli_query($rjdhfbpqj, "SELECT * FROM nota_credito Where anclaje = '$anclaje' and id_cliente = '$id_cliente' and fin='1'");
        ...
        $sqlo3x = mysqli_query($rjdhfbpqj, "SELECT * FROM nota_credito Where id = '$id_orden' and finalizada='1' and fin='1' and col >'2'");
        ...
        // if( $idzona!=$zona){
        $sqlclied = "Update nota_credito Set zona='$zona', poszona='$poszona' Where id = '$id_orden'";
    ...
    if (!empty($id_cliente) && !empty($producto) && !empty($cantidad) && !empty($id_orden)) {
        ...
        $sqlorit = mysqli_query($rjdhfbpqj, "SELECT * FROM item_credito Where id_orden = '$id_orden' and id_producto='$idproduc' and modo='0'");
