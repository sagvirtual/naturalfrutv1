...
if(!empty($_POST['producto']) && $_POST['producto']!=' ' && $_POST['producto']!=null)
  ...
  $sqlorden = mysqli_query($rjdhfbpqj, "SELECT id, nombre, unidadnom, modo_producto, kilo FROM productos Where  $condicion_final ORDER BY `productos`.`nombre` ASC Limit 10");
   while ($roworden = mysqli_fetch_array($sqlorden)) {
            ...
            $nombrevein="<b>".$nombrevd[0]."</b> <br>".$presentacion."<br>".$empresamarca." [Stock: ".$stockdispom." ".$roworden['unidadnom']." / ".$stokbulto[0]." ".$modo_producto."]";
